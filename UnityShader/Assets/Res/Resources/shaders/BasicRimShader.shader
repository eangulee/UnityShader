...
Shader "Custom/Basic Rim Shader" 
{
	...
	Properties
	{
		_MainTex("【主贴图纹理】Main Texture", 2D) = "white" {}
		//主颜色 || Main Color
		...
		Pass
		{
			...
			CGPROGRAM
				...
				fixed4 frag(VertexOutput i) : COLOR
				{
					...
					float4 color = tex2D(_MainTex,TRANSFORM_TEX(i.texcoord.xy, _MainTex));
					color = color * _MainColor;
					float3 finalColor = Diffuse * (tex2D(_TextureDiffuse,TRANSFORM_TEX(i.texcoord.rg, _TextureDiffuse)).rgb * color.rgb) + Emissive;
