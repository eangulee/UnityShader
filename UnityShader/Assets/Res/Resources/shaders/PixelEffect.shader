...
// 核心思路：通过缩放系数确定长宽的像素个数和大小，
// 在uv的x和y上分别使用ceil(uv.x / PixelSize)
// ceil(uv.x / PixelSize)向上取整来离散采用纹理，
// 这样同uv块内的采用一样形成像素块，最终达到整体像素化的效果
Shader "Custom/PostEffect/PixelEffect"
{
	//------------------------------------【属性值】------------------------------------
	Properties
	{
